...
export function NowCard({ role }: { role: NowRole }) {
  return (
    <div className="border border-brand-gray-300 p-8 bg-cream">
      ...
      {role.link && (
        <a href={role.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider mt-4 hover:text-tvgg-rust transition-colors">
          Visit Site →
        </a>
      )}
    </div>
