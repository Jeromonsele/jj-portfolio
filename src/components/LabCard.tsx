...
export function LabCard({ entry }: { entry: LabEntry }) {
  const content = (
    <>
      ...
      <p className="font-mono text-[10px] text-tvgg-rust uppercase tracking-wider mt-4">{entry.tag}</p>
    </>
  );

  const className = "border border-white/10 p-6 cursor-pointer hover:border-tvgg-rust transition-all duration-200 active:scale-[0.98] block";
